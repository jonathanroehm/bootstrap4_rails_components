...
module Bootstrap4Rails
  module Bootstrap
    module Components
      ...
      class Modal < Bootstrap4Rails::Bootstrap::Components::Base
          ...
          super do
            content_tag(:div, class: 'modal-dialog', role: 'document') do
              content_tag(:div, class: 'modal-content') do
                capture do
                  concat(Bootstrap4Rails::Bootstrap::Components::ModalHeader.new({ title: title }, view_context).render)
                  if render_in_body
                    concat(Bootstrap4Rails::Bootstrap::Components::ModalBody.new({ body: (block_given? ? yield : body) }, view_context).render)
                  else
                    concat(block_given? ? yield : body)
                  end
                  if footer
                    concat(Bootstrap4Rails::Bootstrap::Components::ModalFooter.new({ body: footer }, view_context).render)
                  end
                end
              end
            end
