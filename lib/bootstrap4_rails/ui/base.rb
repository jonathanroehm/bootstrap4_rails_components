  ...
  module UI
    ...
    class Base
      ...
      def bootstrap(component_name = nil, *traits, **options, &block)
        Bootstrap4Rails::UI::Bootstrap.new(view_context, component_name, *traits, **options, &block).render_component
      end

      def nfg(component_name = nil, *traits, **options, &block)
        return unless render_nfg_component?(options)
        Bootstrap4Rails::UI::NetworkForGood.new(view_context, component_name, *traits, **options, &block).render_component
      end
