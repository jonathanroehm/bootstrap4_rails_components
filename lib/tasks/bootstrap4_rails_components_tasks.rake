desc "Copies the files in bootstrap4_rails_components's `public/bootstrap4_rails_components` folder to your application's `public` folder."
Rake::Task['assets:precompile'].enhance do
  Rake::Task['bootstrap4_rails_components:copy_assets'].invoke
end

# Enhanced assets:precompile to ensure that the latest assets are copied over
namespace :bootstrap4_rails_components do
  task :copy_assets => :"assets:environment"  do
    puts 'rake bootstrap4_rails_components:copy_assets has begun'
    puts ''
    asset_paths = Dir.glob(Bootstrap4RailsComponents::Engine.root.join('public', 'bootstrap4_rails_components', '**', '*'))
    ...
    puts 'rake bootstrap4_rails_components:copy_assets has completed'
  end
end