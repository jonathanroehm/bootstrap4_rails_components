...
RSpec.describe Bootstrap4Rails::Components::Elements::DropdownItem do
  let(:dropdown_item) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  let(:rendered) { dropdown_item.render }
  it { expect(described_class).to be < Bootstrap4Rails::Bootstrap::Components::DropdownItem }
  ...
  describe '#render' do
    let(:icon_css_selector) { ".text-center.fa-fw.fa-#{icon}.#{Bootstrap4Rails::Components::Foundations::Icon::LEFT_ICON_SPACER_CSS_CLASS}" }
    let(:icon) { nil }
    let(:body) { 'Test body' }
