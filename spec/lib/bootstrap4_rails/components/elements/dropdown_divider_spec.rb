...
RSpec.describe Bootstrap4Rails::Components::Elements::DropdownDivider do
  let(:dropdown_divider) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  it { expect(described_class).to be < Bootstrap4Rails::Bootstrap::Components::DropdownDivider }
