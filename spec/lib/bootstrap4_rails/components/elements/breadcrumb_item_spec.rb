...
RSpec.describe Bootstrap4Rails::Components::Elements::BreadcrumbItem do
  let(:breadcrumb_item) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  it { expect(described_class).to be < Bootstrap4Rails::Bootstrap::Components::BreadcrumbItem }
