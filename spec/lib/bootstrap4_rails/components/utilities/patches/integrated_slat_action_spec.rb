...
RSpec.describe Bootstrap4Rails::Components::Utilities::Patches::IntegratedSlatAction do
  # This utility is consumed by SlatAction
  let(:options) { {} }
  let(:view_context) { ActionController::Base.new.view_context }
  let(:slat_actions) { Bootstrap4Rails::Components::Patterns::SlatActions.new(options, view_context) }

  # Have to run these against SlatActions since it's included in the SlatActions class
  describe 'included modules' do
    let(:described_class) { Bootstrap4Rails::Components::Patterns::SlatActions }
