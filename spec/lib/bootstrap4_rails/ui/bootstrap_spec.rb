...
RSpec.describe Bootstrap4Rails::UI::Bootstrap do
  let(:view_context) { ActionController::Base.new.view_context }
  let(:bootstrap) { Bootstrap4Rails::UI::Bootstrap.new(view_context, component_name, *traits, **options, &block) }
  ...
  describe '#ancestry_string' do
    ...
    it { is_expected.to eq "Bootstrap4Rails::Bootstrap::Components::Alert" }
  end

  describe '#components_within_group(*)' do
    subject { bootstrap.send(:components_within_group) }
    it { is_expected.to eq Bootstrap4Rails::BOOTSTRAP_COMPONENT_NAMES }
  end
end
