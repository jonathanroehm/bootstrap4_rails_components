...
RSpec.describe Bootstrap4RailsComponents::Components::Utilities::Traitable do
  let(:traitable_component) { Bootstrap4RailsComponents::Components::Base.new(options.merge(traits: traits), view_context) }
  let(:view_context) { ActionController::Base.new.view_context }
  let(:options) { {} }
  let(:traits) { [] }

  it { expect(described_class.included_modules).to include Bootstrap4RailsComponents::Components::Traits }
