...
RSpec.describe Bootstrap4RailsComponents::Components::Elements::NavLink do
  let(:nav_link) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  it { expect(described_class).to be < Bootstrap4RailsComponents::Bootstrap::Components::NavLink }
