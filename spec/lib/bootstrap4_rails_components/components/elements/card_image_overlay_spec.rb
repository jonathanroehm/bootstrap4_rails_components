...
RSpec.describe Bootstrap4RailsComponents::Components::Elements::CardImageOverlay do
  let(:card_image_overlay) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  it { expect(described_class).to be < Bootstrap4RailsComponents::Bootstrap::Components::CardImageOverlay }
