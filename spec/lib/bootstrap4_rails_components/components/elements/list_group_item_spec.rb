...
RSpec.describe Bootstrap4RailsComponents::Components::Elements::ListGroupItem do
  let(:list_group_item) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  it { expect(described_class).to be < Bootstrap4RailsComponents::Bootstrap::Components::ListGroupItem }
