...
RSpec.describe Bootstrap4RailsComponents::Components::Elements::ModalBody do
  let(:media_body) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  it { expect(described_class).to be < Bootstrap4RailsComponents::Bootstrap::Components::ModalBody }
