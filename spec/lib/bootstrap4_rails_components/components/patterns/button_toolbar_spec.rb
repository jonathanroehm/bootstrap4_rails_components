...
RSpec.describe Bootstrap4RailsComponents::Components::Patterns::ButtonToolbar do
  let(:button_toolbar) { described_class.new(options, ActionController::Base.new.view_context) }
  let(:options) { {} }
  it { expect(described_class).to be < Bootstrap4RailsComponents::Bootstrap::Components::ButtonToolbar }
