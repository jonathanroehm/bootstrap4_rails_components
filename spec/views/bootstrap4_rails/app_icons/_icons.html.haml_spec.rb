...
RSpec.describe 'app/views/bootstrap4_rails/app_icons/_icons.html.haml', type: :view do
  subject { render partial: 'bootstrap4_rails/app_icons/icons' }

  # for :href key, do not include file extension,
  # DO    -> href: path/to/my_image
  # DON'T -> href: path/to/my_image.png
  let(:apple_touch_icon_attributes) { { href: 'bootstrap4_rails/app_icon/apple-touch-icon', rel: 'apple-touch-icon', sizes: '180x180' } }
  let(:favicon_attributes) { { href: 'bootstrap4_rails/app_icon/favicon', rel: 'shortcut icon' } }
  let(:favicon_32x32_attributes) { { href: 'bootstrap4_rails/app_icon/favicon-32x32', rel: 'icon', sizes: '32x32', type: 'image/png'} }
  let(:favicon_16x16_attributes) { { href: 'bootstrap4_rails/app_icon/favicon-16x16', rel: 'icon', sizes: '32x32', type: 'image/png'} }
  let(:safari_pinned_tab_attributes) { { href: 'bootstrap4_rails/app_icon/safari-pinned-tab', rel: 'mask-icon', color: '#25ACEB' } }
  ...
  it 'includes the site.webmanifest meta tag' do
    expect(subject).to have_css "link[href$='.webmanifest'][href^='/assets/bootstrap4_rails/app_icon/site'][rel='manifest']", visible: :all
  ...
  it 'references browserconfig.xml' do
    expect(subject).to have_css "meta[name='msapplication-config'][content^='/assets/bootstrap4_rails/app_icon/browserconfig']", visible: :all
  end
...
def element_attributes(attributes: {})
  ...
  attributes.each do |k,v|
    ...
    if k == 'href'
      ...
      # [href^='/assets/bootstrap4_rails/app_icon/filename_without_extension']
      k += '^'
      v = "/assets/#{v}"
    end
